...
//we built a balance BST
buildTree [] = Leaf
buildTree list = (Node TakeNode (buildTree Takeleft) (buildTree Takeright))
 where
 sortedlist1 = sort(removeDup list)
 len = length(sortedlist1)
 TakeNode = sortedlist1!!(len/2)
 Takeleft = take(len/2) sortedlist1
 Takeright = drop((len/2) + 1) sortedlist1

//Start = buildTree [1..10]
